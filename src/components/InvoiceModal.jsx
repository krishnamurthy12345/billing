...
const InvoiceModal = ({ onClose, onSave, initialData, clients }) => {
  ...
  const calculateTotal = () => {
    return formData.items.reduce((sum, item) => sum + (item.total || 0), 0).toFixed(2);
  };
          ...
          <div className="items-section">
            ...
            {formData.items.map((item, idx) => (
              <div key={idx} className="item-row">
                ...
                <input type="number" placeholder="Unit Price" value={item.unitPrice} onChange={(e) => updateItem(idx, 'unitPrice', e.target.value)} step="0.01" />
                <span>${(item.quantity * item.unitPrice).toFixed(2)}</span>
                <button type="button" onClick={() => removeItem(idx)} style={{ background: 'none', border: 'none', fontSize: '1.2rem' }}>🗑️</button>
              </div>
            ))}
            <button type="button" className="add-item-btn" onClick={addItem}>+ Add Item</button>
          </div>
          <div style={{ textAlign: 'right', margin: '1rem 0', fontWeight: 'bold', fontSize: '1.2rem' }}>Total: ${calculateTotal()}</div>
